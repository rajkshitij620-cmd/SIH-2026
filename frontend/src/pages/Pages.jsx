...
const allIndiaDestinations = [
 ...
 {id:'dest-amritsar',name:'Golden Temple & Wagah Border',place:'Amritsar, Punjab',city:'Amritsar',category:'Spiritual',rating:'4.9',cost:'1,900',image:'https://upload.wikimedia.org/wikipedia/commons/thumb/9/94/The_Golden_Temple_of_Amritsar_at_Night.jpg/1200px-The_Golden_Temple_of_Amritsar_at_Night.jpg',badge:'Sacred Shrine'},
 ...
 {id:'dest-rishikesh',name:'Lakshman Jhula & Ganga Aarti',place:'Rishikesh, Uttarakhand',city:'Rishikesh',category:'Spiritual',rating:'4.8',cost:'2,000',image:'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e0/Lakshman_Jhula_Rishikesh.jpg/1200px-Lakshman_Jhula_Rishikesh.jpg',badge:'Yoga Capital'},
 {id:'dest-puri',name:'Jagannath Temple & Golden Beach',place:'Puri, Odisha',city:'Puri',category:'Coastal',rating:'4.7',cost:'1,600',image:'https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Jagannatha_Temple%2C_Puri%2C_Odisha.jpg/1200px-Jagannatha_Temple%2C_Puri%2C_Odisha.jpg',badge:'Holy Coast'},
 {id:'dest-hampi',name:'Vijayanagara Ruins & Stone Chariot',place:'Hampi, Karnataka',city:'Hampi',category:'Heritage',rating:'4.9',cost:'2,100',image:'https://upload.wikimedia.org/wikipedia/commons/thumb/4/4b/Stone_chariot_in_Vittala_temple%2C_Hampi.jpg/1200px-Stone_chariot_in_Vittala_temple%2C_Hampi.jpg',badge:'UNESCO Gem'},
 {id:'dest-gangtok',name:'Tsomgo Lake & Rumtek Monastery',place:'Gangtok, Sikkim',city:'Gangtok',category:'Mountains',rating:'4.8',cost:'2,600',image:'https://images.unsplash.com/photo-1584810359583-96fc3448beaa?auto=format&fit=crop&w=900&q=85',badge:'Eastern Himalayas'},
 ...
 {id:'dest-bishnupur',name:'Terracotta Temples & Baluchari',place:'Bishnupur, West Bengal',city:'Bishnupur',category:'Heritage',rating:'4.7',cost:'1,200',image:'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e0/Rasmancha_Bishnupur.jpg/1200px-Rasmancha_Bishnupur.jpg',badge:'Terracotta Art'},
 ...
 return (
  <div className="space-y-16">
   ...
   <div className="relative -mx-5 sm:-mx-8 -mt-16 sm:-mt-20 overflow-hidden bg-slate-950/20 pt-24 sm:pt-28 pb-12 sm:pb-16 lg:pb-20 px-4 sm:px-8 border-b border-slate-200/60 dark:border-slate-800/80">
    ...
    {/* Centered Hero Card (Bigger, Grand & Spacious) */}
    <div className="relative z-10 max-w-5xl mx-auto rounded-3xl bg-gradient-to-br from-teal-50/95 via-white/98 to-emerald-50/90 dark:from-slate-800/95 dark:via-slate-850 dark:to-teal-950/85 p-8 sm:p-14 lg:p-16 border border-teal-100/90 dark:border-teal-700/50 shadow-2xl shadow-slate-950/40 backdrop-blur-md">
     <div className="max-w-4xl">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2 text-xs sm:text-sm text-slate-600 dark:text-slate-300">
   ...
   <div className="shell space-y-16">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
     {[
      {label:'Curated Indian Destinations',value:'500+',icon:Compass},
      {label:'Official Indian Languages',value:'22',icon:Languages},
      {label:'Weather-Adaptive Accuracy',value:'100%',icon:Sun},
      {label:'Crowd Mitigation Score',value:'Low',icon:ShieldCheck}
     ].map((stat,i)=>(
      <div key={i} className="card flex items-center gap-4 !p-5">
       <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
        <stat.icon size={24}/>
       </div>
       <div>
    ...
    {/* 4. 3 Core Technology Pillars in Clean Cards */}
    <div className="grid gap-6 md:grid-cols-3">
     <div className="card">
      <div className="grid h-12 w-12 place-items-center rounded-2xl bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-200 mb-4">
       <Sparkles size={24}/>
      </div>
...
const GuideHighlights=({guide})=>{const stay=guide.best_stay,restaurants=guide.restaurants||[];return <section className="card mt-6"><p className="eyebrow">Complete destination guide</p><h2 className="mt-2 text-xl font-semibold text-slate-900 dark:text-white">Destination guide & recommendations</h2><div className="mt-4 space-y-2 text-sm text-slate-600 dark:text-slate-300">{guide.famous_places?.length>0&&<p><b className="text-slate-900 dark:text-white">Top attractions:</b> {guide.famous_places.join(' · ')}</p>}{guide.best_time?.length>0&&<p><b className="text-slate-900 dark:text-white">Best time to visit:</b> {guide.best_time.join(' · ')}</p>}{guide.weather_advice&&<p><b className="text-slate-900 dark:text-white">Weather advice:</b> {guide.weather_advice}</p>}{guide.local_transportation&&<p><b className="text-slate-900 dark:text-white">Local transport:</b> {guide.local_transportation}</p>}</div><div className="mt-6 grid gap-4 md:grid-cols-2"><article className="rounded-xl border border-teal-200/80 dark:border-teal-900/60 bg-teal-50/70 dark:bg-teal-950/40 p-4"><p className="text-sm font-semibold text-teal-800 dark:text-teal-300">Recommended hotel (within budget)</p>{stay?<><h3 className="mt-2 font-bold text-slate-900 dark:text-white">{stay.name}</h3><p className="mt-1 text-sm font-medium text-slate-700 dark:text-slate-300">₹{stay.price} / night · ★ {stay.rating}</p><p className="mt-2 text-sm text-slate-600 dark:text-slate-400">{stay.description}</p></>:<p className="mt-2 text-sm text-slate-600 dark:text-slate-400">No verified hostel or stay fits this budget yet.</p>}<p className="mt-3 text-xs text-slate-500 dark:text-slate-400">Stay allowance: up to ₹{guide.stay_budget_per_night?.toLocaleString()||0} per night</p></article><article className="rounded-xl border border-amber-200/80 dark:border-amber-900/60 bg-amber-50/80 dark:bg-amber-950/40 p-4"><p className="text-sm font-semibold text-amber-800 dark:text-amber-300">Recommended food option (within budget)</p>{restaurants[0]?<><h3 className="mt-2 font-bold text-slate-900 dark:text-white">{restaurants[0].name}</h3><p className="mt-1 text-sm font-medium text-slate-700 dark:text-slate-300">₹{restaurants[0].price} / person · ★ {restaurants[0].rating}</p><p className="mt-2 text-sm text-slate-600 dark:text-slate-400">{restaurants[0].description}</p></>:<p className="mt-2 text-sm text-slate-600 dark:text-slate-400">No verified restaurant fits this budget yet.</p>}</article></div></section>};
...
const DayWisePlacesChart=({trip})=>{if(!trip?.days?.length)return null;const destName=trip.destination?.name||trip.input?.destination||'Destination',famousPlaces=trip.destination?.famous_places||trip.travel_guide?.famous_places||[];return <section className="card mt-6"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-4"><div><p className="eyebrow">Day-Wise Tour Guide & Chart</p><h2 className="mt-1 text-2xl font-bold text-slate-900 dark:text-white">Famous Places Organised Day-Wise</h2><p className="mt-1 text-sm text-slate-600 dark:text-slate-400">All top attractions and landmarks structured into a daily schedule chart with real-time weather alignment.</p></div><span className="rounded-full bg-teal-100 dark:bg-teal-950 border border-teal-200 dark:border-teal-800 px-3.5 py-1 text-xs font-semibold text-teal-900 dark:text-teal-200">{trip.days.length} Day Schedule Chart</span></div>{famousPlaces.length>0&&<div className="mt-5 rounded-xl border border-teal-100 dark:border-teal-900/60 bg-teal-50/70 dark:bg-teal-950/40 p-4"><p className="text-xs font-semibold uppercase tracking-wider text-teal-900 dark:text-teal-300">Top Famous Attractions Covered in this Destination:</p><div className="mt-2.5 flex flex-wrap gap-2">{famousPlaces.map((place,i)=><span key={i} className="inline-flex items-center gap-1.5 rounded-lg border border-teal-200 dark:border-teal-800 bg-white dark:bg-slate-900 px-3 py-1 text-xs font-semibold text-teal-900 dark:text-teal-200 shadow-sm"><MapPin size={13} className="text-teal-600 dark:text-teal-400"/>{place}</span>)}</div></div>}<div className="mt-6 overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-800"><table className="w-full text-left text-sm"><thead className="border-b border-slate-200 dark:border-slate-800 bg-slate-100/90 dark:bg-slate-900 text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300"><tr><th className="px-4 py-3">Day & Date</th><th className="px-4 py-3">Weather & Suitability</th><th className="px-4 py-3">Key Famous Places</th><th className="px-4 py-3">Morning Landmark (9:30 AM)</th><th className="px-4 py-3">Afternoon Food (1:00 PM)</th><th className="px-4 py-3">Evening Sightseeing (4:30 PM)</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900/70">{trip.days.map((day)=>{const indoor=day.weather?.outdoor_suitability==='low';const morning=day.activities?.find(a=>a.time?.includes('AM')||a.time==='09:30 AM'||a.time==='10:00 AM'||a.category?.includes('Sightseeing')||a.category?.includes('Heritage'));const lunch=day.activities?.find(a=>a.category==='Food'||a.time==='01:00 PM'||a.time==='01:30 PM');const evening=day.activities?.find(a=>a.time?.includes('PM')&&a!==lunch);const dayPlaces=(day.famous_places?.length>0?day.famous_places:[cleanPlace(morning?.place,destName),cleanPlace(evening?.place,destName)]).filter(p=>Boolean(p)&&!p.includes('Museum, gallery')&&!p.includes('craft studio'));return <tr key={day.day} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"><td className="px-4 py-3.5 font-medium whitespace-nowrap"><span className="inline-block rounded bg-teal-900 dark:bg-teal-700 px-2 py-0.5 text-xs font-bold text-white">Day {day.day}</span><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{formatDate(day.date)}</p></td><td className="px-4 py-3.5"><span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium ${indoor?'bg-sky-100 dark:bg-sky-950 text-sky-800 dark:text-sky-300 border border-sky-200 dark:border-sky-800':'bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800'}`}>{indoor?<CloudRain size={12}/>:<Sun size={12}/>}{day.weather?.condition||(indoor?'Indoor Plan':'Good Outdoor')}</span></td><td className="px-4 py-3.5"><div className="flex flex-wrap gap-1.5">{(dayPlaces.length?dayPlaces:[`${destName} Highlights`]).map((p,idx)=><span key={idx} className="rounded bg-teal-50 dark:bg-teal-950/70 px-2 py-0.5 text-xs font-medium text-teal-800 dark:text-teal-200 border border-teal-200 dark:border-teal-800">★ {cleanPlace(p,destName)}</span>)}</div></td><td className="px-4 py-3.5 text-slate-700 dark:text-slate-200"><span className="font-semibold text-teal-950 dark:text-teal-300">{cleanPlace(morning?.place,destName)}</span></td><td className="px-4 py-3.5 text-slate-700 dark:text-slate-200"><span>{lunch?.place&&!lunch.place.includes('well-reviewed')?lunch.place:`${destName} Traditional Kitchen`}</span></td><td className="px-4 py-3.5 text-slate-700 dark:text-slate-200"><span className="font-semibold text-slate-900 dark:text-white">{cleanPlace(evening?.place,destName)}</span></td></tr>})}</tbody></table></div><div className="mt-6 grid gap-4 md:grid-cols-2">{trip.days.map((day)=>{const indoor=day.weather?.outdoor_suitability==='low';const cleanedDayPlaces=(day.famous_places?.length>0?day.famous_places:[cleanPlace(day.activities?.[0]?.place,destName),cleanPlace(day.activities?.[2]?.place,destName)]).filter(p=>Boolean(p)&&!p.includes('Museum, gallery')&&!p.includes('craft studio'));return <article key={day.day} className={`rounded-xl border p-5 ${indoor?'border-sky-200 dark:border-sky-900/60 bg-sky-50/40 dark:bg-sky-950/30':'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900'} shadow-sm`}><div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-3"><div><div className="flex items-center gap-2"><span className="rounded bg-teal-800 dark:bg-teal-700 px-2.5 py-0.5 text-xs font-bold text-white">Day {day.day}</span><h3 className="font-bold text-slate-900 dark:text-white">{day.theme?.includes('Indoor discovery')?'Cultural & Heritage discovery':day.theme}</h3></div><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{formatDate(day.date)}</p></div><span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold ${indoor?'bg-sky-100 dark:bg-sky-950 text-sky-800 dark:text-sky-300 border border-sky-200 dark:border-sky-800':'bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800'}`}>{indoor?<CloudRain size={13}/>:<Sun size={13}/>}{day.weather?.condition||'Clear Weather'}</span></div>{cleanedDayPlaces.length>0&&<div className="mt-3 flex flex-wrap items-center gap-1.5"><span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Day Landmarks:</span>{cleanedDayPlaces.map((p,idx)=><span key={idx} className="rounded bg-teal-50 dark:bg-teal-950/70 px-2 py-0.5 text-xs font-semibold text-teal-800 dark:text-teal-200 border border-teal-200 dark:border-teal-800">★ {cleanPlace(p,destName)}</span>)}</div>}<div className="mt-4 space-y-3">{day.activities?.map((activity,index)=>{const isGeneric=activity.place?.includes('Museum, gallery')||activity.place?.includes('craft studio')||activity.place?.includes('well-reviewed');const placeTitle=cleanPlace(activity.place,destName);const cost=activity.estimated_cost>0?activity.estimated_cost:activity.category==='Food'?300:0;const travelTime=activity.travel_time&&!activity.travel_time.includes('Confirm')?activity.travel_time:'15-20 mins';const whyText=isGeneric?`Recommended ${activity.category?.toLowerCase()} for Day ${day.day} in ${destName}.`:activity.why_recommended;return <div key={index} className="flex items-start gap-3 rounded-lg border border-slate-100 dark:border-slate-800/80 bg-slate-50/80 dark:bg-slate-800/60 p-3"><div className="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-md bg-teal-100 dark:bg-teal-950 text-teal-800 dark:text-teal-300 font-bold text-xs">{activity.time?.slice(0,2)||'09'}</div><div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-1"><b className="text-sm text-slate-900 dark:text-white">{placeTitle}</b><span className="text-xs font-medium text-slate-500 dark:text-slate-400">{activity.time}</span></div><p className="mt-0.5 text-xs text-teal-800 dark:text-teal-300 font-medium">{activity.category} {cost>0&&`· ₹${cost}`} {`· ⏱️ ${travelTime}`}</p><p className="mt-1 text-xs text-slate-600 dark:text-slate-300">{whyText}</p></div></div>})}</div></article>})}</div></section>};
...
const CURATED_COMMUNITY_TRAVELERS = [
  {
    traveller: { id: 't-101', name: 'Aarav Sharma', avatar_url: '', is_premium: true, bio: 'Photographer & mountain trekker' },
    trip: { id: 'ft-1', destination: 'Jaipur, Rajasthan', current_location_city: 'Delhi', gender: 'male', age: 26, start_date: '2026-10-15', end_date: '2026-10-19', budget: 12000 },
    match_percentage: 94,
    connection_status: null
  },
  {
    traveller: { id: 't-102', name: 'Pooja Iyer', avatar_url: '', is_premium: true, bio: 'Heritage walker and food explorer' },
    trip: { id: 'ft-2', destination: 'Varanasi, Uttar Pradesh', current_location_city: 'Bangalore', gender: 'female', age: 24, start_date: '2026-11-01', end_date: '2026-11-05', budget: 9500 },
    match_percentage: 89,
    connection_status: null
  },
  {
    traveller: { id: 't-103', name: 'Rohan Verma', avatar_url: '', is_premium: false, bio: 'Solo traveler seeking road trip buddies' },
    trip: { id: 'ft-3', destination: 'Manali, Himachal Pradesh', current_location_city: 'Chandigarh', gender: 'male', age: 28, start_date: '2026-10-20', end_date: '2026-10-25', budget: 15000 },
    match_percentage: 86,
    connection_status: null
  },
  {
    traveller: { id: 't-104', name: 'Ananya Deshmukh', avatar_url: '', is_premium: true, bio: 'Beach lover & yoga enthusiast' },
    trip: { id: 'ft-4', destination: 'Goa', current_location_city: 'Mumbai', gender: 'female', age: 25, start_date: '2026-11-10', end_date: '2026-11-15', budget: 18000 },
    match_percentage: 91,
    connection_status: null
  }
];
...
export function FindTravelers(){
  ...
  const fetchMatches=async(tripId='')=>{
    ...
    try{
      ...
      setData({ matches: [], same_destination_travelers: CURATED_COMMUNITY_TRAVELERS });
    }finally{
      setLoad(false);
    }
  };
  ...
  let allTravelers=[...rawMatches,...rawSameDest.filter(s=>!rawMatches.some(m=>m.traveller.id===s.traveller.id))];
  
  if(allTravelers.length===0){
    allTravelers = CURATED_COMMUNITY_TRAVELERS;
  }
  ...
  return (
    <div className="shell max-w-5xl py-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <button className="btn text-xs sm:text-sm" onClick={()=>nav('/plan')}>
          + Plan New Trip
        </button>
      </div>
...
export const getTripTypeLabel=(trip,group,is_shared)=>{
 if(is_shared) return 'TravelMate Tour';
 if(group) return 'TravelMate Group';
 const travelType=trip?.input?.travel_type;
 const travellers=Number(trip?.input?.travellers)||1;
 const connectionOption=trip?.input?.connection_option;
 if(travelType==='group'||travellers>1) return 'Group & Family Tour';
 if(connectionOption==='connect_people') return 'TravelMate Tour';
 return 'Solo Tour';
};

export const getTripTypeDetails=(trip,group,is_shared)=>{
 if(is_shared) return 'Joined TravelMate Group';
 if(group) return `TravelMate Group (${group.member_ids?.length||2} members)`;
 const travelType=trip?.input?.travel_type;
 const travellers=Number(trip?.input?.travellers)||1;
 const connectionOption=trip?.input?.connection_option;
 if(travelType==='group'||travellers>1) return `${travellers} travellers · Group & Family Plan`;
 if(connectionOption==='connect_people') return 'TravelMate Matchmaking';
 return 'Solo Explorer · Personal plan';
};

export function ItineraryView({trip,onSave,allTrips=[],selectedId,onTripChange}){const nav=useNavigate();if(!trip)return <div className="shell py-12">Building your itinerary…</div>;const destination=trip.destination?.name||trip.input?.destination||'Destination',days=trip.input?.days||trip.days?.length||1;const typeLabel=getTripTypeLabel(trip);const typeDetails=getTripTypeDetails(trip);return <div className="shell py-10"><div className="flex items-center gap-2"><p className="eyebrow">{typeLabel}</p><span className="text-xs px-2.5 py-0.5 rounded-full font-medium bg-teal-50 text-teal-700 dark:bg-teal-950 dark:text-teal-300 border border-teal-200 dark:border-teal-800">{typeDetails}</span></div><div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="mt-2 text-3xl font-bold">{destination} · {days} days</h1><p className="mt-1 text-sm text-slate-500">{formatDate(trip.input?.start_date)} – {formatDate(trip.input?.end_date)} · ₹{trip.input?.budget?.toLocaleString()}</p></div><div className="flex flex-wrap items-center gap-2">{allTrips.length>1&&<label className="flex items-center text-xs font-medium text-slate-500">Choose trip:<select className="input ml-1.5 min-w-44 !py-1.5 !px-2 text-xs" value={selectedId||trip.id} onChange={e=>onTripChange?.(e.target.value)}>{allTrips.map(item=><option value={item.trip.id} key={item.trip.id}>{item.trip.destination?.name||item.trip.input?.destination} · {formatDate(item.trip.input?.start_date)} ({getTripTypeLabel(item.trip,item.group,item.is_shared)})</option>)}</select></label>}<button onClick={()=>nav('/plan')} className="btn-ghost">Edit</button><button onClick={()=>onSave?.(trip.id)} className="btn"><Save size={16}/> {trip.saved?'Saved':'Save Tour'}</button></div></div>{trip.ai_summary&&<div className="card mt-6 border-teal-200 bg-teal-50"><p className="eyebrow">AI trip brief</p><p className="mt-2">{trip.ai_summary}</p><p className="mt-2 text-sm text-slate-600">{trip.ai_recommendation_reason}</p></div>}<LiveMap location={destination} trip={trip}/>{trip.budget_breakdown&&<aside className="card mt-6"><p className="eyebrow">Budget estimate</p><div className="mt-4 grid gap-3 sm:grid-cols-4">{Object.entries(trip.budget_breakdown).map(([k,v])=><div key={k} className="rounded-lg bg-stone-50 p-3"><p className="text-xs uppercase text-slate-500">{k}</p><p className="mt-1 text-lg font-semibold text-teal-800">₹{Math.round(v).toLocaleString()}</p></div>)}</div></aside>}</div>}
