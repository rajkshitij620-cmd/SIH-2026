   ...
   <header className={`site-header sticky top-0 z-40 transition-all duration-300 ${
    ...
    <div className="flex items-center gap-3">
     ...
     <div className="relative" ref={settingsRef}>
      ...
      {settingsOpen && (
       <>
        <div className="fixed inset-0 z-40 bg-slate-950/40 backdrop-blur-xs md:hidden" onClick={()=>setSettingsOpen(false)}/>

        <div className="fixed left-3 right-3 top-20 sm:absolute sm:left-auto sm:right-0 sm:top-full mt-2 w-[calc(100vw-24px)] sm:w-[320px] max-h-[82vh] overflow-hidden flex flex-col rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl z-50 animate-in fade-in zoom-in-95 duration-150">
         {/* Minimal Header */}
         <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-800">
          <span className="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200">Preferences</span>
          <button 
           type="button" 
         ...
         <div className="px-4 pt-3 pb-2">
          <p className="text-[11px] font-medium text-slate-400 dark:text-slate-500 mb-1.5">Theme</p>
          <div className="grid grid-cols-2 gap-1 bg-slate-100 dark:bg-slate-950 p-1 rounded-xl border border-slate-200/70 dark:border-slate-800">
           <button
            type="button"
            onClick={()=>theme!=='light'&&toggleTheme()}
            className={`flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg text-xs transition-all ${
             theme === 'light' 
              ? 'bg-white text-slate-900 shadow-xs font-semibold border border-slate-200/70' 
           ...
           >
            <Moon size={14} className={theme === 'dark' ? 'text-teal-300' : 'text-slate-400'} />
            <span>Dark</span>
           </button>
          </div>
         </div>
         ...
         {/* Language Section */}
         <div className="px-4 pt-2 pb-3.5 flex flex-col flex-1 min-h-0">
          ...
          <div className="relative mb-2">
           ...
           <input
            ...
            className="w-full rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 pl-7.5 pr-2.5 py-1.5 text-xs text-slate-800 dark:text-slate-200 placeholder:text-slate-400 focus:border-teal-500 focus:bg-white dark:focus:bg-slate-900 focus:outline-none transition"
           />
          </div>
          ...
          <div className="max-h-48 overflow-y-auto space-y-0.5 pr-1">
           {languages
             ...
             .map((l)=>(
              ...
              >
               <span>{l.label}</span>
               {currentLang === l.name && <Check size={13} className="text-teal-600 dark:text-teal-400" />}
              </button>
           ))}
          </div>
         </div>
